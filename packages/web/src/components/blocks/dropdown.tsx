    ...
    <div className={cn(props.index > 0 && 'mt-6')}>
      ...
      >
        <BaseSelect.Trigger
          className={cn(
            // Base
            'flex h-14 w-full items-center justify-between rounded-(--radius) border-2 border-border px-4 text-base text-foreground transition-all outline-none',
            // Focus
            'focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50',
            props.static && 'pointer-events-none',
          )}
        >
          <BaseSelect.Value className="data-placeholder:text-foreground/50">
            {(value) => {
              ...
              <BaseSelect.List>
                {options.map((option) => (
                  <BaseSelect.Item
                    key={option.id}
                    value={option.id}
                    className="grid cursor-default scroll-my-1 grid-cols-[1fr_0.75rem] items-center gap-2 rounded-[calc(var(--radius)-3px)] py-3.5 pr-5 pl-3 leading-4 text-foreground transition-colors outline-none select-none data-highlighted:bg-muted"
                  >
